    ...
    static notifications = [
        {
            ...
            message: 'Deployment terbaru Anda telah selesai dengan sukses.',
        ...
        },
        {
            id: 2,
            type: 'warning',
            title: 'Peringatan Beban Server',
            message: 'Beban server pada 92% kapasitas. Pertimbangkan untuk mengoptimalkan.',
            time: '5 menit yang lalu',
            unread: true,
            icon: 'exclamation-circle',
            actions: ['Lihat Detail', 'Tutup']
        },
        {
            id: 3,
            type: 'info',
            title: 'Pendaftaran Pengguna Baru',
            message: 'Gustiawan Asep telah mendaftar sebagai pengguna baru.',
...
class ActivityManager {
    static activities = [
        ...
        {
            user: 'Asep Gustiawan',
            action: 'Menyelesaikan tugas: UI Dasbor',
            time: '10 menit yang lalu',
            icon: 'check-circle',
            color: '#2ed573'
        }
    ...
    static init() {
        ...
        if (profileTrigger && profileDropdown) {
            ...
            if (themeToggle) {
                ...
                if (savedTheme === ThemeManager.THEMES.LIGHT) {
                    document.documentElement.classList.replace(ThemeManager.THEMES.DARK, ThemeManager.THEMES.LIGHT);
                    ThemeManager.currentTheme = ThemeManager.THEMES.LIGHT;
                    themeToggle.checked = false;  // unchecked for light (not dark mode)
                } else {
                    // Default dark theme
                    document.documentElement.classList.add(ThemeManager.THEMES.DARK);
                    ThemeManager.currentTheme = ThemeManager.THEMES.DARK;
                    themeToggle.checked = true;  // checked for dark mode
                }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (themeToggleSettings) {
        const savedTheme = localStorage.getItem('theme');
        if (savedTheme === ThemeManager.THEMES.LIGHT) {
            themeToggleSettings.checked = false;  // unchecked for light (not dark mode)
        } else {
            themeToggleSettings.checked = true;  // checked for dark mode
        }
